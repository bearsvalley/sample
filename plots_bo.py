...
#        plt.figure(figsize=[10,6])
        ...
        plt.plot(x_grid, m, 'k-',lw=2,alpha = 0.6)
        plt.plot(x_grid, m-1.96*np.sqrt(v), 'k-', alpha = 0.2)
        plt.plot(x_grid, m+1.96*np.sqrt(v), 'k-', alpha=0.2)
        plt.plot(Xdata, Ydata, 'k.', markersize=10,alpha=0.6)
        plt.axvline(x=suggested_sample[len(suggested_sample)-1],color='b',alpha=0.6)
        factor = max(m+1.96*np.sqrt(v))-min(m-1.96*np.sqrt(v))
        plt.plot(x_grid,0.2*factor*acqu_normalized-abs(min(m)),'b',lw=1,alpha=0.6)
        plt.plot(x_grid,0.2*factor*acqu_normalized-abs(min(m-np.sqrt(v))),'b--',lw=1,alpha=0.2)
        plt.plot(x_grid,0.2*factor*acqu_normalized-abs(min(m+np.sqrt(v))),'b--',lw=1,alpha=0.2)
        plt.xlabel('x')
        plt.ylabel('f(x)')
        plt.grid()
